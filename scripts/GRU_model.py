...
def gru_model(X_train, y_train, X_test, y_test, n_timesteps, n_features, num_layers, units_per_layer, 
              learning_rate=0.001, epochs=20, batch_size=32):
    """
    GRU model for time series prediction with configurable layers, learning rate, epochs, and batch size.
    ...
    - units_per_layer (list of int): Number of units in each GRU layer.
    ...
    # Reshape the data for GRU (3D shape: [samples, timesteps, features])
    ...
    # Add GRU layers dynamically based on `num_layers` and `units_per_layer`
    for i in range(num_layers):
        if i == num_layers - 1:  # Last layer should not return sequences
            model.add(GRU(units_per_layer[i], activation='relu', input_shape=(n_timesteps, n_features)))
        else:  # Intermediate layers should return sequences
            model.add(GRU(units_per_layer[i], activation='relu', return_sequences=True))
